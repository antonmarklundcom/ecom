...
export const metadata: Metadata = {
  title: TEXTOS.cuenta.entrarTitulo,
...
export default async function EntrarPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-md px-4 py-10">
      <h1 className="text-xl font-semibold tracking-tight">{TEXTOS.cuenta.entrarEncabezado}</h1>
      <p className="text-muted-foreground mt-1 text-sm">{TEXTOS.cuenta.entrarAyuda}</p>
      ...
      {conCodigo ? (
        <div className="border-border mt-6 border-t pt-6">
          <h2 className="text-sm font-medium">{TEXTOS.cuenta.olvidasteContrasena}</h2>
          <p className="text-muted-foreground mt-1 mb-4 text-sm">{TEXTOS.cuenta.olvidasteAyuda}</p>
          <CodigoAccesoForm />
        </div>
      ) : null}

      <p className="text-muted-foreground mt-6 text-sm">
        {TEXTOS.cuenta.todaviaNoTenesCuenta}{" "}
        <Link href="/cuenta/registro" className="underline">
          {TEXTOS.cuenta.creaUna}
        </Link>
        .
      </p>
      ...
      <p className="text-muted-foreground border-border mt-6 border-t pt-4 text-xs">
        {TEXTOS.cuenta.noHaceFaltaCuentaInvitada}
      </p>
    </main>
  );
}
