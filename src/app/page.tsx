      ...
      */}
      {TIENDA.hero ? (
        ...
        <section className="border-border bg-muted/30 rounded-2xl border p-6 sm:p-10">
          <h1 className="text-2xl font-semibold tracking-tight sm:text-4xl">
            {TEXTOS.home.heroTitulo}
          </h1>
          <p className="text-muted-foreground mt-3 max-w-xl text-sm sm:text-base">
            {TEXTOS.home.heroTexto}
          </p>
          <div className="mt-6 flex flex-wrap gap-3">
            {categories[0] ? (
              <Button asChild size="lg">
                <Link href={`/categoria/${categories[0].slug}`}>{TEXTOS.comunes.verProductos}</Link>
              </Button>
            ) : null}
          </div>
        </section>
      ...
      {error ? (
        <div className="border-border border-l-primary mt-8 rounded-lg border border-l-2 p-4">
          <p className="text-sm">{TEXTOS.home.errorCatalogo}</p>
          <p className="mt-1 font-mono text-xs break-all">{error}</p>
          <p className="text-muted-foreground mt-2 text-sm">
            {TEXTOS.home.errorCatalogoAyuda("docker compose up -d", "pnpm db:push && pnpm db:seed")}
          </p>
        </div>
      ) : (
        <>
          {categories.length > 0 ? (
            <section className="mt-10">
              <h2 className="text-lg font-semibold">{TEXTOS.comunes.categorias}</h2>
              <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                {categories.map((category) => (
                  ...
                  >
                    ...
                    <p className="text-muted-foreground mt-1 text-sm">{TEXTOS.comunes.verTodo}</p>
                  </Link>
                ))}
              </div>
            </section>
          ...
          {featured.length > 0 ? (
            <section className="mt-12">
              <div className="flex items-baseline justify-between">
                <h2 className="text-lg font-semibold">{TEXTOS.home.destacados}</h2>
              </div>
            ...
            <p className="text-muted-foreground mt-10 text-sm">
              {TEXTOS.home.sinProductos} <code>pnpm db:seed</code>.
            </p>
          )}
        </>
      )}
